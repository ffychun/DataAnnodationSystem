...
def search_tasks(request):
    taskType = request.GET.getlist('taskType')
    taskRank = request.GET.getlist('taskRank')
    perTaskPaid = request.GET.getlist('perTaskPaid')
    taskID = request.GET.get('taskID', '')
    taskTitle = request.GET.get('taskTitle', '')

    # 构建查询条件
    query = {}
    if taskType:
        query['taskType__in'] = taskType
    if taskRank:
        query['taskRank__in'] = taskRank
    if perTaskPaid:
        query['perTaskPaid__in'] = perTaskPaid
    if taskID:
        query['taskId'] = taskID
    if taskTitle:
        query['taskTitle__icontains'] = taskTitle
